...
{
    public class GeneratorWebApiNgx
    {
        /// <summary>
...
export interface HttpResponse extends Response {
  error: any;
        ...
        public GeneratorWebApiNgx(TsImportsManager importsManager)
        {
        ...
        {
            if (this.TsGenConfig == null)
                throw new ArgumentException($"Must supply a {nameof(TsGenConfig)} property!");
...
";
        ...
        private string Generate_webApiCall(EndpointMethodSpec endpointMethod)
        {
            ...
            var bodyParamPassed = (endpointMethod.HttpMethod == "get" || endpointMethod.HttpMethod == "delete")
                ? ""
                : (endpointMethod.BodyParams.FirstOrDefault()?.Name ?? "null");
            if (endpointMethod.BodyParams.FirstOrDefault()?.ParamType == typeof(string))
                bodyParamPassed = bodyParamPassed.TemplateIfNotNull("JSON.stringify($1)");
